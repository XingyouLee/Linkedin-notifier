...
export default function DashboardPage() {
  ...
  const isGenerationAvailable = !statusLoading && Boolean(systemStatus?.generation_available);
  const masterResumeStorageKey = systemStatus?.workspace_id
    ? `master_resume_id:${systemStatus.workspace_id}`
    : 'master_resume_id';

  const isTailorEnabled =
    Boolean(masterResumeId) && processingStatus === 'ready' && isGenerationAvailable;
    ...
    async (resumeId: string) => {
      try {
        ...
        if (err instanceof Error && err.message.includes('404')) {
          localStorage.removeItem(masterResumeStorageKey);
  ...
  useEffect(() => {
    ...
    const storedId = localStorage.getItem(masterResumeStorageKey);
    if (storedId) {
    ...
    setMasterResumeId(null);
    setProcessingStatus('loading');
  }, [checkResumeStatus, masterResumeStorageKey, statusLoading]);
  ...
  const loadTailoredResumes = useCallback(async () => {
    try {
      ...
      if (resolvedMasterId) {
        localStorage.setItem(masterResumeStorageKey, resolvedMasterId);
  ...
  }, [checkResumeStatus, masterResumeStorageKey]);
  ...
  const handleUploadComplete = (resumeId: string) => {
    localStorage.setItem(masterResumeStorageKey, resumeId);
  ...
  return (
    <div className="space-y-6">
      ...
      {masterResumeId && !isGenerationAvailable && !statusLoading && (
      ...
      <SwissGrid>
        ...
        {!masterResumeId ? (
          ...
          !isGenerationAvailable && !statusLoading ? (
            <Card className="aspect-square h-full border-dashed border-warning bg-amber-50">
