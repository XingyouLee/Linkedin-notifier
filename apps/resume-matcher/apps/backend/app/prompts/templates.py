...
DUTCH_STYLE_GUIDANCE = (
    "Write for Dutch companies and Dutch HR review while keeping the requested "
    "output language and using a direct, concise, modest, low-fluff style."
)


def _with_dutch_style_guidance(template: str) -> str:
    """Embed shared Dutch-style wording without adding a new .format key."""
    return template.replace("{dutch_style_guidance}", DUTCH_STYLE_GUIDANCE)
...
IMPROVE_RESUME_PROMPT_NUDGE = _with_dutch_style_guidance(
    """Lightly nudge this resume toward the job description. Output ONLY the JSON object, no other text.
...
IMPROVE_RESUME_PROMPT_KEYWORDS = _with_dutch_style_guidance(
    """Enhance this resume with relevant keywords from the job description. Output ONLY the JSON object, no other text.
...
IMPROVE_RESUME_PROMPT_FULL = _with_dutch_style_guidance(
...
COVER_LETTER_PROMPT = _with_dutch_style_guidance(
...
DIFF_IMPROVE_PROMPT = _with_dutch_style_guidance(
...
8. {dutch_style_guidance}
9. Do not use em dash characters
10. Keep changes minimal and targeted — do not rewrite content that already aligns well
