...
class TestLlmConfig:
    ...
    async def test_workspace_session_cannot_access_shared_settings(
        self,
        make_workspace_session_cookie,
    ):
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as workspace_client:
            cookie_name, cookie_value = make_workspace_session_cookie()
            workspace_client.cookies.set(cookie_name, cookie_value)
            resp = await workspace_client.get("/api/v1/config/llm-api-key")

        assert resp.status_code == 403
        assert resp.json()["detail"] == "Settings are not available inside workspace sessions."
