...
from app.workspace_auth import WORKSPACE_SESSION_COOKIE_NAME, build_workspace_session_token
...
@pytest.fixture
def make_workspace_session_cookie(monkeypatch):
    """Build a valid workspace session cookie for protected API routes."""
    monkeypatch.setenv("RESUME_MATCHER_SESSION_SECRET", "test-session-secret")

    def _make(workspace_id: str = "test-workspace", ttl_seconds: int = 3600) -> tuple[str, str]:
        token = build_workspace_session_token(
            workspace_id=workspace_id,
            expires_at=int(time.time()) + ttl_seconds,
        )
        return WORKSPACE_SESSION_COOKIE_NAME, token
